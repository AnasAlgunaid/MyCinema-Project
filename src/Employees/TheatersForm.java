 ...
 * @author anasm
 */
public class TheatersForm extends javax.swing.JInternalFrame {
    
    public static void refreshTable(){
        String allTheatersQuery = "SELECT Theater_Number, Theater_Types.Type_Name AS `Theater_Type`, Number_of_Seats AS `Number of seats` FROM Theaters INNER JOIN Theater_Types ON Theaters.Theater_Type = Theater_Types.type_ID ";
        
        // Check for the order combobox 
        if(orderComboBox.getSelectedItem() == "Number: Ascending"){
            allTheatersQuery = allTheatersQuery + " ORDER BY Theater_Number ASC";
        }else if(orderComboBox.getSelectedItem() == "Number: Descending"){
            allTheatersQuery = allTheatersQuery + " ORDER BY Theater_Number DESC";
        }
        else if(orderComboBox.getSelectedItem() == "Seats: Ascending"){
            allTheatersQuery = allTheatersQuery + " ORDER BY Number_of_Seats ASC";
        }else if(orderComboBox.getSelectedItem() == "Seats: Descending"){
            allTheatersQuery = allTheatersQuery + " ORDER BY Number_of_Seats DESC";
        }

        try(
                    Statement selectAllTheaters = EmployeeSystem.connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                    ResultSet theatersResultSet = selectAllTheaters.executeQuery(allTheatersQuery);
                ){

            theatersTable.setModel(DbUtils.resultSetToTableModel(theatersResultSet));
            
            // Set the number of rows in the records label
            int numOfRows = theatersTable.getRowCount();
    ...
    private void initComponents() {
        ...
        theatersTable = new javax.swing.JTable();
        ...
        theatersTable.setFont(new java.awt.Font("Segoe UI", 0, 16)); // NOI18N
        theatersTable.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null},
                {null, null, null},
                {null, null, null}
            ...
            new String [] {
                "Theater_Number", "Theater_Type", "Number_Of_Seats"
            }
        ) {
        ...
        theatersTable.setFocusable(false);
        theatersTable.setRowHeight(40);
        theatersTable.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        theatersTable.setShowGrid(true);
        jScrollPane1.setViewportView(theatersTable);
        ...
        searchComboBox.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Theater Number", "Theater Type" }));
        ...
        jLabel3.setText("<html>Theaters Management</html>");
        ...
        orderComboBox.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Number: Ascending", "Number: Descending", "Seats: Ascending", "Seats: Descending" }));
    ...
    private void addButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addButtonActionPerformed
        NewAndUpdateTheater newAndUpdateTheater = new NewAndUpdateTheater(null, true);
        newAndUpdateTheater.setVisible(true);
    }//GEN-LAST:event_addButtonActionPerformed

    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        // Get the selected row
        int selectedRow = theatersTable.getSelectedRow();
        if(selectedRow >=0){
            ...
            if(confirm == JOptionPane.YES_OPTION){
                ...
                final String DELETE_QUERY = "DELETE FROM Theaters WHERE Theater_Number = ?";

                // Get the selected theater number from the table
                int selectedTheaterNumber = (Integer) theatersTable.getModel().getValueAt(selectedRow, 0);
                try(PreparedStatement deleteStatement = EmployeeSystem.connection.prepareStatement(DELETE_QUERY)){
                    deleteStatement.setInt(1, selectedTheaterNumber);
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        ...
        if(selectedRow >= 0){
            ...
            NewAndUpdateTheater newAndUpdateTheater = new NewAndUpdateTheater(null, true);
            newAndUpdateTheater.setSelectedTheaterNumber(selectedTheaterNumber);
            newAndUpdateTheater.setVisible(true);
    ...
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
        String searchQuery = "SELECT Theater_Number, Theater_Types.Type_Name AS `Theater_Type`, Number_of_Seats AS `Number of seats` FROM Theaters INNER JOIN Theater_Types ON Theaters.Theater_Type = Theater_Types.type_ID WHERE ";
        
        // Search by Theater number or Theater type
        if(searchComboBox.getSelectedItem() == "Theater Number"){
            try{
                int theaterNumber = Integer.parseInt(searchTextField.getText());
                searchQuery = searchQuery + " Theater_Number = " + searchTextField.getText();
            }catch(NumberFormatException e){
                JOptionPane.showMessageDialog(null, "Theater Number must be a number, Please enter a valid number", "Invalid Theater Number", JOptionPane.ERROR_MESSAGE);
                return;
            }
            
        }else if(searchComboBox.getSelectedItem() == "Theater Type"){
            searchQuery = searchQuery + "Theater_Types.Type_Name LIKE \'" + searchTextField.getText() +"%\' ";
        }

        // Check for the order combobox 
        if(orderComboBox.getSelectedItem() == "Number: Ascending"){
            searchQuery = searchQuery + " ORDER BY Theater_Number ASC";
        }
        else if(orderComboBox.getSelectedItem() == "Number: Descending"){
            searchQuery = searchQuery + " ORDER BY Theater_Number DESC";
        }
        else if(orderComboBox.getSelectedItem() == "Seats: Ascending"){
            searchQuery = searchQuery + " ORDER BY Number_of_Seats ASC";
        }else if(orderComboBox.getSelectedItem() == "Seats: Descending"){
            searchQuery = searchQuery + " ORDER BY Number_of_Seats DESC";
        }     
            ...
            ResultSet searchResultSet = getSearchResult.executeQuery(searchQuery);)
        {
            ...
            // Refresh the table
            theatersTable.setModel(DbUtils.resultSetToTableModel(searchResultSet));
    ...
    protected static javax.swing.JTable theatersTable;
